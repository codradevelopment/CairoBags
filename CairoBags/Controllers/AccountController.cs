...
namespace CairoBags.Controllers
{
   ...
   public class AccountController : ControllerBase
   {
       ...
       public async Task<IActionResult> CreateAdmin([FromBody] CreateAdminDto dto)
       {
           ...
           if (!disableAuthorization && !hasAnyAdmin && !isDevelopment)
           {
               var expectedKey = _configuration["AdminBootstrapKey"];
               if (string.IsNullOrEmpty(expectedKey) || !Request.Headers.TryGetValue("X-Bootstrap-Key", out var providedKey) || providedKey != expectedKey)
               {
                   return Unauthorized(new { message = "An AdminBootstrapKey must be configured and provided via X-Bootstrap-Key header to create the first admin in production." });
               }
           }
