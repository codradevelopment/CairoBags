...
export function UserDropdown({ className }) {
  ...
  return (
    <div ref={rootRef} className={cn("relative", className)}>
      ...
      >
        <UserIcon />
        <span className="hidden max-w-[8rem] truncate sm:inline">{isAuthenticated ? displayName : labels.login}</span>
      </Button>
        ...
        >
          {isAuthenticated ? (
            <>
              ...
              <nav className="py-1">
                <a href="/account" className="block px-4 py-2.5 text-sm hover:bg-brand-secondary" role="menuitem">
                  {labels.account}
                </a>
                <a href="/account/orders" className="block px-4 py-2.5 text-sm hover:bg-brand-secondary" role="menuitem">
                  {labels.orders}
                </a>
                {isAdmin ? (
                  <a href="/admin" className="block px-4 py-2.5 text-sm hover:bg-brand-secondary" role="menuitem">
                    {labels.admin}
                  </a>
                ) : null}
              </nav>
            ...
            <nav className="py-1">
              <a href="/login" className="block px-4 py-2.5 text-sm hover:bg-brand-secondary" role="menuitem">
                {labels.login}
              </a>
              <a href="/register" className="block px-4 py-2.5 text-sm hover:bg-brand-secondary" role="menuitem">
                {labels.register}
              </a>
            </nav>
          )}
